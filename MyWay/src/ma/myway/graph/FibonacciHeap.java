...
public final class FibonacciHeap<T> {
	...
	public static <T> FibonacciHeap<T> merge(FibonacciHeap<T> one, FibonacciHeap<T> two) {
		...
		/*
		 * Merge the two Fibonacci heap root lists together. This helper function also
		 * computes the min of the two lists, so we can store the result in the mMin
	...
	public Entry<T> dequeueMin() {
		...
		/*
		 * Otherwise, we're about to lose an element, so decrement the number of entries
		 * in this heap.
		 */
		--mSize;
		...
		/*
		 * Now, we need to get rid of this element from the list of roots. There are two
		 * cases to consider. First, if this is the only element in the list of roots,
		 * we set the list of roots to be null by clearing mMin. Otherwise, if it's not
		 * null, then we write the elements next to the min element around the min
		 * element to remove it, then arbitrarily reassign the min.
		 */
		...
		/*
		 * Next, clear the parent fields of all of the min element's children, since
		 * they're about to become roots. Because the elements are stored in a circular
		 * list, the traversal is a bit complex.
		 */
		...
		/*
		 * Next, splice the children of the root node into the topmost list, then set
		 * mMin to point somewhere in that list.
		 */
		...
		/*
		 * Next, we need to coalsce all of the roots so that there is only one tree of
		 * each degree. To track trees of each size, we allocate an ArrayList where the
		 * entry at position i is either null or the unique tree of degree i.
		 */
		List<Entry<T>> treeTable = new ArrayList<Entry<T>>();

		/*
		 * We need to traverse the entire list, but since we're going to be messing
		 * around with it we have to be careful not to break our traversal order
		 * mid-stream. One major challenge is how to detect whether we're visiting the
		 * same node twice. To do this, we'll spent a bit of overhead adding all of the
		 * nodes to a list, and then will visit each element of this list in order.
		 */
		...
		for (Entry<T> curr : toVisit) {
			...
			while (true) {
				...
				/*
				 * Determine which of the two trees has the smaller root, storing the two tree
				 * accordingly.
				 */
			...
			/*
			 * Update the global min based on this node. Note that we compare for <= instead
			 * of < here. That's because if we just did a reparent operation that merged two
			 * different trees of equal priority, we need to make sure that the min pointer
			 * points to the root-level one.
			 */
	...
	public void delete(Entry<T> entry) {
		/*
		 * Use decreaseKey to drop the entry's key to -infinity. This will guarantee
		 * that the node is cut and set to the global minimum.
		 */
	...
	private static <T> Entry<T> mergeLists(Entry<T> one, Entry<T> two) {
		...
		if (one == null && two == null) { // Both null, resulting list is null.
			...
			/*
			 * This is actually not as easy as it seems. The idea is that we'll have two
	...
	private void decreaseKeyUnchecked(Entry<T> entry, double priority) {
		...
		/*
		 * If the node no longer has a higher priority than its parent, cut it. Note
	...
	private void cutNode(Entry<T> entry) {
		...
		/*
		 * If the node is the one identified by its parent as its child, we need to
		 * rewrite that pointer to point to some arbitrary other child.
		 */
		...
		/*
		 * Splice this tree into the root list by converting it to a singleton and
		 * invoking the merge subroutine.
		 */
